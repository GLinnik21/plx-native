...
#[cfg_attr(not(test), allow(dead_code))] // Phase 8: emitted by the upcoming owned Home Tick.
...
impl StoreWork {
    pub(crate) fn store(self) -> StoreId {
        match self { Self::Hubs => StoreId::Hubs, Self::BrowseDiscovery => StoreId::Browse }
    }
}

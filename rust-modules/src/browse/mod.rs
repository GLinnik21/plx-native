...
/// A store bookmark names a media item or an explicitly unloaded slot, never an engine cursor.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum CursorAt {
    Item { sid: ServerId, rk: String },
    SlotIndex(usize),
}

pub(crate) fn save_section_view(section: usize, at: Option<CursorAt>, scroll: f32) {
    let Some(state) = state_mut(section) else { return };
    if let Some(at) = at {
        state.focus = match &at {
            CursorAt::Item { sid, rk } => (0..state.total.max(0) as usize)
                .find(|&index| state.items.get(index).is_some_and(|item| item.sid == *sid && item.rk == *rk))
                .unwrap_or(state.focus),
            CursorAt::SlotIndex(index) => *index,
        };
        if state.cursor.as_deref() != Some(&at) { state.cursor = Some(Arc::new(at)); }
    }
    state.scroll = scroll;
}

...
fn coldstart_frame(
    ...
    if crate::coldstart::restore_pending() {
        ...
        if matches!(
            page_of(*route),
            Route::Login | Route::Profiles | Route::Onboard
        ) {
            return;
        }
...
fn coldstart_place(route: Route) -> Option<crate::coldstart::PlaceRef<'static>> {
    ...
    let item = |sid: crate::plex::ServerId, rk: &'static str| {
        let machine = crate::plex::client_for(sid)
            .map(|c| c.machine_id())
            .unwrap_or("");
    ...
    match page_of(route) {
        ...
        Route::Library => Some(PlaceRef::Library {
            tab: crate::browse::tab_of_section(crate::browse::cur()) as u32,
        }),
...
unsafe fn key_onboarding(route: Route, sym: c_uint, wcode: c_uint, ok_armed: &mut bool) -> bool {
    if matches!(route, Route::Onboard) {
        ...
        return onboarding_action_leaves(crate::ui::onboard::key(sym, wcode));
    }
...
/// Both commit and cancel leave the Settings-hosted Home editor. First-run never emits `Cancel`,
/// but treating it as unfinished here trapped the Settings variant on `Route::Onboard` forever.
...
pub extern "C" fn plex_run(pms_host: *const c_char, pms_port: c_int) -> c_int {
    ...
    unsafe {
        ...
        // dev: /tmp/plxnative-settings=<root|privacy|legal> opens the Settings modal (and,
                        ...
                        if is_ok(sym) {
                            ...
                            if crate::ui::consent::take_delete_request() {
                                match delete_all_local_data() {
                                    ...
                                    Err(e) => crate::log(&format!("privacy: local-data deletion failed: {e}")),
                                }
                            }
                        } else if is_back(sym, wcode) {

...
fn route_wears_tab_bar(r: Route) -> bool {
    match r {
        ...
        Route::Account => true,
        // …and the popover inherits the answer from the screen it is drawn ON, which is the only
        // way this can stay right as hosts are added: a menu over the Library wears the bar because
        // the Library does.
...
pub extern "C" fn plex_run(pms_host: *const c_char, pms_port: c_int) -> c_int {
    ...
    unsafe {
        ...
        while running {
            ...
            // back into motion when the menu closes. The detail page already said so in a comment
            // and spelled it as a second `matches!` arm; asking `page_of` says it once and is what
            // lets a new `MenuHost` land without editing five gates.
            ...
            if present {
                ...
                crate::ui::profile::phase("frame.ui", || {
                    ...
                    if player {
                        ...
                        // **`page_of`, not the bare route.** Every screen stays live UNDER its
                        // context menu — the popover is anchored beside a tile that has to still be
                        // there to be beside, and its scrim lifts that tile back out of the dim —
                        // so the dispatch asks which PAGE a route draws rather than naming the
                        // popover cases one at a time. Only the detail arm ever did, which is why
                        // the Library, Search and person page all fell through to `home_draw` the
                        // moment they became menu hosts.

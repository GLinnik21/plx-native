...
enum Reach {
    ...
    /// trying them buys nothing. Reporting "can't reach <peer-name-2>" here would send the user to look
...
mod tests {
    ...
    fn a_share() -> Resource {
        resource(
            r#"{"name":"<peer-name-2>","clientIdentifier":"bbbb2222","provides":"server","owned":false,
                "sourceTitle":"<peer-owner-1>","publicAddressMatches":false,"httpsRequired":false,
    ...
    /// trying them wastes 2 s each and then reports "can't reach <peer-name-2>", sending the user to look
    ...
    fn a_two_server_account() -> Vec<Resource> {
        serde_json::from_str(
            r#"[
              {"name":"<peer-name-2>","clientIdentifier":"bbbb2222","provides":"server","owned":false,
    ...
    fn a_sign_in_to_a_two_server_account_settles_on_one_address_each_ours_first() {
        ...
        assert_eq!(share.shared_by, "<peer-owner-1>");
    ...
    fn source(machine_id: &str, owned: bool, token: &str) -> SourceRef {
        SourceRef {
            ...
            shared_by: if owned { String::new() } else { "<peer-owner-1>".into() },
            owned,
    ...
    fn switching_profile_re_keys_every_source_and_drops_the_ones_not_granted() {
        ...
        assert_eq!(next[1].shared_by, "<peer-owner-1>", "everything but the token is carried over");

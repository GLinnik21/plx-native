...
fn maybe_spawn() {
    ...
    // …and so is the SERVER. `key` alone is ambiguous across sources (both servers have a section
    // `1`), and `client()` inside the worker would answer with whatever is current by then.
    let Some(sid) = section_sid(c) else { return };

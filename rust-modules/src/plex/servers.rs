...
pub struct ServerFacts {
    /// The MACHINE name — "<peer-name-2>". "People in content, machines in settings": this is drawn
    /// only where the grant itself is the subject, i.e. the Sources list's group headers.
    pub name: String,
    /// The owner's plex.tv handle — "<peer-owner-1>". **Empty on your own server**, and empty is the
...
mod tests {
    ...
    fn the_roster_walks_in_registration_order_and_describing_it_never_blanks_a_known_field() {
        ...
        describe(b, "", "<peer-owner-1>", false);
        describe(b, "<peer-name-2>", "", false);
        let f = facts(b).expect("described");
        assert_eq!((f.name.as_str(), f.handle.as_str(), f.owned), ("<peer-name-2>", "<peer-owner-1>", false));

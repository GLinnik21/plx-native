...
pub(crate) fn pump(ps: &mut crate::route::PlaybackSession, pa: &mut super::adapter::PlayerAdapter, now: u32) {
    ...
    // completion arm below: an abandoned trailer-preview Load must not drive any of this —
    // no deferral logging, no budget clock, nothing that reacts to it as if it were live.
    ...
    } else if eng.stage == Stage::Loading && !eng.preview_abandon {

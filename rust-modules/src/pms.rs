...
struct HubRow {
    ...
    /// Which SERVER this shelf's items came from, as the owner's handle ("<peer-owner-1>") — empty
...
/// was drawn from came from ("<peer-owner-1>") — empty for the signed-in user's own, exactly as
...
/// Handle of the server hero-pool page `i` came from ("<peer-owner-1>"), or **empty** for the signed-in
...
/// Handle of the server hub `i` came from ("<peer-owner-1>"), or **empty** for the signed-in user's own
...
mod tests {
    ...
    fn a_borrowed_page_never_opens_the_door_while_we_have_one_of_our_own() {
        let mut p = pool_of(&["<peer-owner-1>", "<peer-owner-1>", "", "", "ldn"]);
        own_items_first(&mut p);
        assert_eq!(sources_of(&p), ["", "<peer-owner-1>", "<peer-owner-1>", "", "ldn"], "one page is promoted, nothing else moves");
    ...
    fn a_pool_that_already_opens_on_one_of_ours_is_left_exactly_alone() {
        for spelling in [vec!["", "", ""], vec!["", "<peer-owner-1>", ""], vec!["", "<peer-owner-1>"]] {
    ...
    fn a_borrowed_only_account_still_gets_a_hero_and_it_holds_the_first_rotation() {
        let mut p = pool_of(&["<peer-owner-1>", "ldn", "<peer-owner-1>"]);
        own_items_first(&mut p);
        assert_eq!(sources_of(&p), ["<peer-owner-1>", "ldn", "<peer-owner-1>"], "nothing of ours to promote — leave the pool as it is");
    ...
    fn the_ordering_holds_at_the_empty_and_single_page_ends() {
        ...
        for one in [vec![""], vec!["<peer-owner-1>"]] {

...
impl DocumentReader {
    pub(crate) const fn new() -> Self {
        Self { scroll: Spring::at(0.0), target: 0.0, max_scroll: 0.0 }
    }
    ...
    pub(crate) fn draw(&mut self, p: Painter, frame: Rect, title: Option<&str>, body: &str) {
        ...
        for line in body.lines() {
            ...
            let h = if trimmed.is_empty() { BODY_LEAD } else { view.measure_h((text_w - indentation).max(1.0)) };
        ...
        for (line_y, h, indentation, view) in &lines {
            ...
            if top + h < body_frame.y || top > body_frame.y + body_frame.h { continue; }
            view.draw(
                p,
                Rect::new(

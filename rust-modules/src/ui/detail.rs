...
/// The trailing SOURCE credit's words — `Shared by <peer-owner-1>`, or **empty** for an item that lives on
/// the signed-in user's own server.
///
/// **The person, never the machine.** `<peer-owner-1>` is the same handle the shelf headings and the
/// Sources list say; the server's own name (`<peer-name-2>`) appears nowhere outside that list and the
...
/// the trailing `\u{b7} Shared by <peer-owner-1>`.
///
...
mod tests {
    ...
    fn the_source_credit_is_the_last_run_on_the_line_after_the_play_mode_fragment() {
        let credit = super::shared_by(HANDLE);
        assert_eq!(credit, "Shared by <peer-owner-1>", "the person, never the machine");

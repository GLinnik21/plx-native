...
//! | `LDN Films` | `<peer-owner-1> · 1 hr 57 min`       | `4K`    |
//!
...
pub(crate) struct AltCopy {
    ...
    /// Libraries are what a person browses; the machine name (`<peer-name-2>`) belongs to the Sources
...
/// one that must not elide: "<peer-owner-1> · 1 hr 57 min" is the answer to *whose copy is this*.
...
pub(crate) struct AltRow {
    ...
    /// `"This account · 1 hr 57 min"` / `"<peer-owner-1> · 1 hr 57 min"`
...
mod tests {
    ...
    fn the_button_appears_only_when_a_second_source_holds_the_item() {
        ...
        assert_eq!(
            source_count(&[copy(0, "Movies", "", "4", "1080"), copy(1, "LDN Films", "<peer-owner-1>", "318", "4k")]),
            2
        );
        ...
        assert_eq!(
            source_count(&[
                ...
                copy(1, "LDN Films", "<peer-owner-1>", "318", "4k"),
                copy(1, "Films", "<peer-owner-1>", "319", "1080"),
            ]),
            2
        );
    }
    ...
    fn the_copy_that_plays_comes_first_and_the_rest_rank_by_preference() {
        let list = [copy(1, "LDN Films", "<peer-owner-1>", "318", "4k"), copy(0, "Movies", "", "4", "1080")];
        ...
        assert_eq!(rs[1].detail, "<peer-owner-1> \u{b7} 1 hr 57 min");
    ...
    fn quality_outranks_ownership_and_ownership_breaks_the_tie() {
        ...
        let theirs_4k = copy(1, "LDN Films", "<peer-owner-1>", "318", "4k");
    ...
    fn exactly_one_row_is_ever_ticked() {
        ...
        let list = [copy(0, "Movies", "", "4", "1080"), copy(1, "LDN Films", "<peer-owner-1>", "318", "4k")];
        let rs = rows(&list, sid(0), "4");
    ...
    fn a_copy_with_no_runtime_states_only_its_owner() {
        let mut c = copy(1, "LDN Films", "<peer-owner-1>", "318", "4k");
        c.dur_ms = 0;
        let rs = rows(&[c], sid(0), "4");
        assert_eq!(rs[0].detail, "<peer-owner-1>");

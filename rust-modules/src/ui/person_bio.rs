...
//! 1120×700 glass sheet with 48px of padding and a [`theme::ALERT_RAD`] corner, holding an
...
/// The one padding, on all four sides. Quoted with [`theme::ALERT_RAD`] because the two constrain
...
/// The footer's right-hand hint, as its three runs. `BACK` is drawn as a [`widgets::keycap`] — the
...
pub(crate) fn draw() {
    ...
    pop().panel(p, panel, theme::ALERT_RAD);
...
fn draw_foot(p: Painter, person: &Person, c: Rect) {
    ...
    let key_w = widgets::keycap_w(HINT_KEY);
    ...
    widgets::keycap(p, kx, cy, HINT_KEY, theme::TEXT_SECONDARY);

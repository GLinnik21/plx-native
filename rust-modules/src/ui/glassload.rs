...
pub(crate) fn draw() {
    ...
    for (i, r) in layout(step).into_iter().enumerate() {
        match step.kind {
            Kind::Glass => {
                if p.backdrop_blur(r, 0.0, PANEL_RADIUS, [1.0, 1.0, 1.0, 1.0], crate::gfx::GlassRim::Bevelled, crate::gfx::GlassFace::NONE) {
...
pub(crate) fn draw_nav_blur() -> bool {
    ...
    let drew = p.backdrop_blur(full, 0.0, 0.0, [1.0, 1.0, 1.0, amount], crate::gfx::GlassRim::Bevelled, crate::gfx::GlassFace::NONE);

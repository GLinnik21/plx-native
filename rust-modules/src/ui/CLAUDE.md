...
| `item_menu.rs` | The **press-and-hold card context menu** (Apple TV's card popover, NOT Plex's full-screen sheet): a `Popover`+`TableView` panel anchored BESIDE the focused home card — the card and shelf stay in place behind it — with `[icon] [label]` rows, a `Row::separator()` between the navigation and state groups, and the row set adapted per item kind. It only REPORTS an `Action`; `app.rs`'s `apply_item_action` does the routing, the scrobble and the hub refresh. Opened off `press::is_long` (`Route::ItemMenu`); a SHORT press still plays a Continue Watching tile immediately, which is intentional. |
| `person.rs` | The **person / actor page** (Apple-TV shape, not Plex's field list): centred circular portrait → name → roles → Born/Died → an optional 3-line bio with an inline `MORE` → plain `Movies` and `Shows` shelves of ordinary poster cards, all flowed through a `ScrollColumn` whose children are the header and the PRESENT shelves. Opened by OK on any **credits** headshot — cast or crew alike, since a director is a person with a `tagKey` too (`detail.rs`'s section-4 arm, via `metadata::credit(i)`); its data is `crate::person`. Two things to know before editing it. **(1) The header is a FOCUS ROW** (flow child 0, exactly as detail's hero is section 0) even though it holds no control: it is the page's top scroll position, and without it UP from the first shelf had nowhere to go, which left the portrait permanently off screen once the header outgrew the room a focused shelf leaves. **(2) Every header line below the name is optional and comes from plex.tv** (`plex/discover.rs` — the LOCAL server has no biography; that much of the old note was right, the conclusion "so Plex has none" was not). A line is drawn only when it has content and the block height is measured from what is present, so the no-bio page reads as finished rather than broken — do not "fix" it with placeholder lines or a header spinner. |

...
| `fmt.rs` | The shared display formatters — `dur_short` ("2h 15m") / `dur_long` / `time_left` / `clock` / `episode_kicker` / `rating_score` (a review score back in its own provider's units: RT + TMDB are percentages, IMDb is out of ten, while PMS sends all three as 0–10). ONE home for these, replacing the "2 hr 15 min" vs "2h 15m" vs "0 hr 45 min" drift across screens. Format a duration here, never inline. |
...
| `person.rs` | The **person / actor page** (Apple-TV shape, not Plex's field list): centred circular portrait → name → an optional 3-line bio with an inline `MORE` → plain `Movies` and `Shows` shelves of ordinary poster cards, all flowed through a `ScrollColumn` whose children are the header and the PRESENT shelves. Opened by OK on any **credits** headshot — cast or crew alike, since a director is a person with a `tagKey` too (`detail.rs`'s section-4 arm, via `metadata::credit(i)`); its data is `crate::person`. **The no-bio state is the normal one** — PMS has no biography to give, so the header is composed to read as finished at name + portrait; do not "fix" it with a placeholder line. |

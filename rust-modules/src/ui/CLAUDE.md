...
| `widgets.rs` | reusable leaves: `Button` (+`ControlStyle`, +`progress` — a left-to-right fill sweep that makes the pill its own timer; drawn as the face twice under a scissor so the label's ink flips at the sweep line instead of going illegible over half of it), `TabPill` (+`TabStyle`), `TransportButton`, `CircleButton`, `Spinner`, `PageDots`, `StatusOverlay` (+`StatusKind` — the Working/Failed treatment the player draws over Connecting/Buffering/Seeking/Error; the *caller* supplies the caption so the state machine stays the single source of that string), the shared art-tile core (`card`/`draw_card` + `Art`) and the watched-state marks that ride it — the poster's amber corner angle (inside `card`) and `watched_badge`, the disc-and-check a landscape still wears. One vocabulary in one hue (amber `RESUME_*`): corner angle = never started, resume bar = in progress, check = done, never two at once. A new tile type's state mark belongs here, not in the screen — plus the poster-resolve helper `resolve_tex` and the layout companion `pill_w` (the ONE hero-pill width formula — both hero rows relabel their pill from item state, home's "Play"/"Continue" and detail's "Play"/"Resume", so the frame is measured from the label rather than fixed). **There is still no `ProgressBar` type**: the HUD scrubber is immediate-mode inside `player_hud.rs` (see the deliberately-immediate-mode gotcha below), and the product's one other bar — the Up Next countdown — is `Button::progress`, a fill sweep *inside* the pill rather than a rail beside it. If you need a standalone bar, that is the moment to promote one here, not to assume it exists. |
| `widgets.rs` | reusable leaves: `Button` (+`ControlStyle`, +`progress` — a left-to-right fill sweep that makes the pill its own timer; drawn as the face twice under a scissor so the label's ink flips at the sweep line instead of going illegible over half of it), `TabPill` (+`TabStyle`), `TransportButton`, `CircleButton`, `Spinner`, `PageDots`, `StatusOverlay` (+`StatusKind` — the Working/Failed treatment the player draws over Connecting/Buffering/Seeking/Error; the *caller* supplies the caption so the state machine stays the single source of that string), the shared art-tile core (`card`/`draw_card` + `Art`), the review-score badge `rating_badge`/`rating_badge_w` (a brand mark tinted `theme::RATING_*` + the score — deliberately NOT a bordered `badge` chip), plus the poster-resolve helper `resolve_tex`. **There is still no `ProgressBar` type**: the HUD scrubber is immediate-mode inside `player_hud.rs` (see the deliberately-immediate-mode gotcha below), and the product's one other bar — the Up Next countdown — is `Button::progress`, a fill sweep *inside* the pill rather than a rail beside it. If you need a standalone bar, that is the moment to promote one here, not to assume it exists. |

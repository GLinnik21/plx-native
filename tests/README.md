...
an 8 Mbit/s Original, must detect both sustained low transfer rate and a drained content-time
buffer, replace it with a sustainable fixed HLS rung, and later climb to at least 4 Mbit/s after
the link recovers. It must then reach the 20 Mbit/s top rung, pass two spaced probes against the
actual Original fixture, perform a third fresh Load back onto Original, and re-arm the progressive
watchdog. The assertion requires that full order; a startup at low quality, a fallback without
recovery, an upshift logged before the collapse, or stopping at “changed up to 20” all fail.

The HLS master/media playlists are generated by `serve_fixtures.py` and their independent two-second
H.264/AAC MPEG-TS segments come from `pipe_abr_240p.ts`, `pipe_abr_480p.ts`, `pipe_abr_720p.ts`, and
`pipe_abr_1080p.ts`. No PMS decision, real token, library item, transcoder, or Internet path participates.

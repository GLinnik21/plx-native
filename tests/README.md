...
gate — three healthy segments — usually beats the ladder's five, so requiring an upshift first would
have been a race the case lost for the wrong reason.
...
`pipe_abr_1080p.ts` — four clips covering every rung a 1080p source can request, because what a rung
asks PMS for is a bitrate CEILING and the raster is the consequence. No PMS decision, real token,
library item, transcoder, or Internet path participates.

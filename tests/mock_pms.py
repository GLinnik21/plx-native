...
class Library:
    ...
    def _probe(self, path):
        ...
        try:
            ...
            raise ValueError("ffprobe is required with --media") from e
    ...
    def _verified_media(self, path, rk, part_id):
        ...
        part = {
            "id": part_id, "key": f"/library/parts/{part_id}/1/file.mkv",
        ...
        media = {
            ...
            "videoCodec": video["codec"], "videoResolution": "1080", "container": "mkv",
            "videoFrameRate": "24p", "Part": [part],
        }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _media(self, method):
        ...
        if len(segs) < 3 or segs[:2] != ["library", "parts"] or not segs[2].isdigit():
            return False
        path = getattr(self.server.pms.lib, "media_files", {}).get(int(segs[2]))
        if path is None:
            return False
        size = path.stat().st_size

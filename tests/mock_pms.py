...
class Library:
    ...
    def __init__(self, seed=1, movies=48, shows=6, seasons=2, episodes=6):
...
class MockPms:
    ...
    def handle(self, method, path, body=b""):
        ...
        if len(segs) == 4 and segs[:2] == ["library", "sections"]:
            ...
            if d == "firstCharacter":
                return j(self.container(Directory=[{"key": "s", "title": "S", "size": 1}]))
...
def serve(port, seed=1, host="127.0.0.1", verbose=False):

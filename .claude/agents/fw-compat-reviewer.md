...
* **Always pass `--min-release 4.4.2`: a bare `tools/fwcompat.py` EXITS 1 on a perfectly healthy
  ...
  instead of the table gets a regression that is not one. CLAUDE.md's own example line
...
`curl_easy_getinfo` once. CLAUDE.md, `dynlib!`'s own doc and `net.rs`'s own comment all say "one C
...
`.claude/skills/which-tier/` is the general form of that question.
...
`.claude/skills/decompile-tv-lib/` — harvest the actual `.so` off a set and read it — and for a
new binding, `.claude/skills/bind-tv-lib-abi/`. Name whichever applies in your verdict rather than

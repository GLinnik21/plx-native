...
description: >
  ...
  "check the docs are still right", "does AGENTS.md still match", "does
  docs/agent-reference.md still match", "did I break any documentation",
  ...
  one), greps the prose surfaces that actually exist here — AGENTS.md,
  docs/agent-reference.md, the three nested module guides, the READMEs, the notes under docs/,
...
down in the past tense. Go and read them; they are all still in `docs/agent-reference.md`, greppable by phrase:
...
- **The host test count rotted three times.** "386 measured 2026-08-13; 284 on 2026-08-02; a
  ...
  the same batch that documented it." docs/agent-reference.md now **refuses to state a count at all** and opens
...
compiles the project's Markdown guidance.** `make check` cannot fail on it, clippy cannot see it,
and CI does not read it.
...
3. **`.agents/skills/*/SKILL.md`** — count them yourself, `ls -d .agents/skills/*/ | wc -l`. This
...
`.claude/worktrees/` (`.gitignore:31`), each holding its own older copy of `docs/agent-reference.md` and all of
...
git log -S 'the exact old sentence' -- docs/agent-reference.md    # when did this claim become false?
```
...
**Exclude this file.** `.agents/agents/` is not gitignored, and the "Why this exists" section above
...
`git grep -n -- '<phrase>' -- '*.md' ':!.agents/agents'` — a verified pathspec, and the first thing
...
1. **Docs carrying the heartbeat mapping banner.** Five notes —
   ...
   rather than live guidance." docs/agent-reference.md ratifies this: those docs "carry a mapping banner instead
...
2. **Notes that mark themselves historical or superseded.** `docs/async-model-review.md` §3b opens
   ...
   **One asymmetry to check rather than assume**: `docs/buffer-feed-plan.md` is labelled "(partly
   outdated)" **in docs/agent-reference.md's key-files list, not in its own header** — the file itself opens with
   no marker at all. So a reader who arrives by search rather than via docs/agent-reference.md gets no warning.
   Do not report its stale interior (it still describes the `zig c++` build and the stub-`.so`
   trick, both retired), but a *missing banner on a doc docs/agent-reference.md calls outdated* is itself a
   legitimate, one-line finding if you are already reporting something else nearby.
3. **Hedged approximations.** docs/agent-reference.md says "~40" dev triggers; `dev.rs`'s module doc says "~44".
...
- **The doc that documents its own rot.** Several paragraphs here *quote the old wrong sentence*
  in order to correct it. `git grep 'no chunked decoding'` hits docs/agent-reference.md — inside the sentence
...
- **Do not put a fresh count into your own proposed replacement** unless the number is genuinely
  ...
  to. Prefer the shape docs/agent-reference.md and `ci.yml` both settled on: name the command that produces the
  number.
- **Quote a greppable phrase, not only a line number.** `docs/agent-reference.md` is edited most
  weeks; a bare line reference in a report read tomorrow can point at a different sentence. Give
  both.
...
```
1. docs/agent-reference.md:553  (tier 2 — shared detailed reference)
...
what lands, and an auditor that silently rewrites docs/agent-reference.md produces a diff nobody reviewed against
...
```
1. docs/agent-reference.md:646  (tier 2 — shared detailed reference)
...
1. rust-modules/README.md:16  (tier 4 — crate onboarding doc)
   ...
   TRUTH:   rust-modules/src/mkv.rs does not exist; it was deleted in 8715572c (2026-07-18,
            ...
            Root docs/agent-reference.md already says so: "`ff.rs` is the only demux path." Line 22's `ui_home`

 ...
 * Included by main.c (single TU, all static). Scope: H264 (V_MPEG4/ISO/AVC),
 * unlaced SimpleBlock/Block, known element sizes (real Plex remuxes qualify). */
...
static int ebml_id(mkv_ctx *c, unsigned int *id, int *idlen) {
    unsigned char b0;
...
static int ebml_size(mkv_ctx *c, long long *size, int *szlen) {
    unsigned char b0;
    if (msrc_read(c, &b0, 1) != 1) return 0;
    int len = 1; unsigned char mask = 0x80;
...
static void mkv_parse_cues(mkv_ctx *c, long long size) {
    long long consumed = 0;
    while (size < 0 || consumed < size) {
        unsigned int id; int il; long long sz; int sl;
        if (!ebml_id(c, &id, &il)) break;
...
static void mkv_seek_run(mkv_ctx *c) {

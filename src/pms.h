 ...
 * JSON over HTTP (stream.h) and scrapes each movie into pms_movies[]. The JSON is
 * flat-ish and regular, so per-item string/number scraping suffices (same spirit
 * as the mkv.h / json scrapers) — no full JSON parser needed for a POC. */
#include <string.h>
#include <stdlib.h>
#include <stdio.h>
#include "stream.h"
...
static pms_movie pms_movies[PMS_MAX_MOVIES];
static int       pms_nmovies = 0;
...
static int pms_fetch_movies(const char *host, int port, const char *token, int sec) {
    http_stream hs;
    char path[160];

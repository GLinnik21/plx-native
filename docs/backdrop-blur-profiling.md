...
|---|---|---|---|
| Account panel (608x396) | 10,107,885 | 10,086,395 | **−0.21%** |
| Account + glass tab bar (1324x456) | 10,453,654 | 10,428,664 | **−0.24%** |

**The direct path is worth about 0.2% of the frame, at both region sizes.** An earlier revision of
this note claimed 3.7%; that figure came from reading the last few `PROFILE` log lines instead of
the sample distribution, and did not survive a proper interleaved A/B. The whole-frame quads and
...
The main UI is the other 88.6%, at **3.65x overdraw** for 1080p. Measured frame rate is identical
across all three legs (50 fps each in one session), so on this hardware 14% more GPU work costs
zero frames — the cap is not the GPU.

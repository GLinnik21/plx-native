...
Never a reassurance we cannot support: there is no update push here, and telemetry is opt-in and carries no account identity (a crash report's random Crash report ID counts opted-in televisions, nothing more), so *"no evidence of misuse"* and *"few users affected"* stay unsupportable — an opt-in "users affected" figure is a sample of the people who opted in, which is not a population and must never be quoted as one. The honest sentence is that we cannot tell whether this happened to you, which is why the instruction has no conditions on it.

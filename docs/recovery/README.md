...
Source checkpoint: `c38dbe7363f5824e581f818ba7a4fab554e78ca0` on `codex/ui-phase12-resume`.
This supersedes earlier checkpoint descriptions. Phases 0-11 are on local main. This snapshot includes phase-12 deletion/gates, shared chrome ownership (7 named render caches), corrected replay counts, and strict generic replay-frame decoding. Chrome passed independent review, parent host/shipping/lab/ARM checks, inspected synthetic simulator captures, and the three existing live-assisted replay anchors. The generic decoder passed worker full checks plus parent focused tests, shipping and ARM checks. The harness reports 3 matching anchors and 9 skips, not 11 passes.

This is recovery, not a release or completion claim. Concrete Session ownership, closed product bootstrap/result replay and side-effect suppression, product targets/resolve modes, full scenario/device proof and carried scheduling findings remain unfinished. The ongoing auth-worker observation conversion is NOT included. Original dirty Claude worktrees remain preserved locally. No phase-12 device proof is claimed; simulator pixels are not device performance evidence.

## Latest recovery checkpoint (2026-09-11)

Source checkpoint: `2b1936e81eab38c6a784feed4d2e93176e986dec` on `codex/ui-phase12-resume`.
This supersedes earlier checkpoint descriptions. Phases 0-11 are on local main. This snapshot includes phase-12 deletion/gates, shared chrome ownership (7 named render caches), corrected replay counts, and strict generic replay-frame decoding. Chrome passed independent review, parent host/shipping/lab/ARM checks, inspected synthetic simulator captures, and the three existing live-assisted replay anchors. The generic decoder passed worker full checks plus parent focused tests, shipping and ARM checks. The harness reports 3 matching anchors and 9 skips, not 11 passes.

This is recovery, not a release or completion claim. Concrete Session ownership, closed product bootstrap/result replay and side-effect suppression, product targets/resolve modes, full scenario/device proof and carried scheduling findings remain unfinished. The ongoing auth-worker observation conversion is NOT included. Original dirty Claude worktrees remain preserved locally. No phase-12 device proof is claimed; simulator pixels are not device performance evidence.

This checkpoint also includes R2Q1 exact-address bounded one-shot Landing admission and metadata caller correction. Capacity rejection is synchronous, accepted requests reserve one terminal, and cancelled completions are sequenced and retired on the main-thread drain. Three regressions failed at runtime before the fix. Worker full checks passed (2880 default / 2902 hostsim Rust tests; 286 harness tests), shipping and lab passed; parent review, 15 Landing / 22 metadata / 9 Detail identity tests, shipping and ARM build passed after integration. This does not include the ongoing auth-worker conversion or the subsequent progress-stream extension. Completed phases 8-11 are now also on origin/main; phase12 remains separate.

...
| **I9** | **BLOCKED** | §7.A's argmax and §7.B's quality scoring. Unblocked only by M3 **and** a 4K `auto_network` case. | — | — |
...
| `E_tx` | unrefilled playback an **upshift** transaction costs | ms, wall | ~4 600 (2.3·d at d = 2 s) | **derived, unmeasured** | elapsed between prime and commit | **yes, cheaply** | **M4** |
...
| `upshift_dwell_ms` | encoder-lifecycle guard (**operational**) | ms, wall | TBD from `E_tx` | derived | commits per minute | **yes** | I6's A/B |
| `reject_backoff_ms` | failed-prime rate limit (**operational**) | ms, wall | TBD from `E_tx` | derived | reject → re-prime interval | **yes** | I6's A/B |

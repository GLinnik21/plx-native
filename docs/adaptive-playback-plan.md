...
`app/diagnostics.rs` diagnostics; the host and device test tiers; `docs/adaptive-playback.md`. Transport,
...
(`abr.rs:1692`), the panel (`ff.rs:2808` → `app/diagnostics.rs:673-674`) and one test (`abr.rs:3159`). The
...
- **Render with the scale.** `app/diagnostics.rs:673-674` prints `" · risk {}"` bare on the surface whose
...
`app/diagnostics.rs:518` paints *any* finite starvation horizon as a fault (`.fault(d.abr_starve_secs >=

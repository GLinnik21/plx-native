...
- **No subtitle appearance controls (size / position / style)** — `major` / `medium`  
  The official client lets the user set subtitle size, position and style. Every one of ours is a compile-time constant: size 36, a fixed bottom baseline, pure white with a black outline, 3 lines max at 42 chars. ASS positioning is actively discarded: `{\anN}` override blocks are stripped in the text extractor, so a top-positioned sign renders at the bottom over the dialogue. The timing offset half has landed — see "No subtitle timing (sync) offset" below (`ui::track_menu`'s Timing section, `player::subtitle_offset_range_ms`).  
  ...
  *Verified:* CONFIRMED; one evidence nit. ui/player_hud.rs:81-92 is all literals: `let sz = 36`, `let lh = 48.0`, `let baseline = if hud_up { SCR_H - 300.0 } else { SCR_H - 100.0 }`, `let white = [1.0,1.0,1.0,1.0]`, `wrap(seg, 42)`, `lines.len() < 3`, and a fixed 4-offset 2px black outline. NIT on the evidence: the ASS-override strip at player/mod.rs:314 is `'{' => whi
...
- **No subtitle timing (sync) offset** — `major` / `small`  
  ...
  *Landed (lands #243):* a **Timing** section under Color in the player's Subtitles menu (`ui::track_menu`) — Earlier / Later in 100 ms steps and Reset, the offset as the section's read-out, up to 30 s late for any track and 30 s early for a sidecar; an embedded track takes a delay only, its Earlier row drawn dim at zero (`player::subtitle_offset_range_ms`, which the menu and the player's clamp both call) — held for one playback of one track: it is never persisted, and a new item or a different subtitle track starts at zero, because a timing error belongs to one track against one file. Every client-rendered lookup (embedded text, image sets, the sidecar) goes through `player::subtitle_clock_ns` (the playhead less the offset). Hidden on a transcode, where the server has already burned the captions. An embedded track's cues ride the same byte-bounded A/V queues as the picture (about 2 s ahead of the playhead at a high bitrate), so an advance would find nothing to draw; a sidecar is whole in memory.  

...
- **No language preference for audio or subtitles, and the account's own preferences are never fetched** — `major` / `medium`  
  Plex honours the account's defaultAudioLanguage / defaultSubtitleLanguage / autoSelectSubtitle / defaultSubtitleForced, so a user whose account says "Japanese audio, English forced subs" gets that everywhere. We hard-code a single preferred audio language ("eng") in a const, have NO subtitle language preference at all, and never auto-enable subtitles — not even forced tracks on a foreign-audio item, which every Plex client does.  
  *Where:* rust-modules/src/plex/account.rs (new GET https://plex.tv/api/v2/user via net.rs), stored beside rust-modules/src/plex/session.rs, consumed by rust-modules/src/route.rs:657/677-698 (audio ladder) and a new subtitle auto-select in route.rs:586-615.  
  *Device:* The plex.tv fetch needs DNS+TLS, which the raw-socket stream.rs cannot do — but net.rs/libcurl already carries every account call (plex/account.rs:52-66), so this is an existing, proven seam. Auto-enabling a forced sub costs nothing on direct-play (client renderer); doing it on a transcode costs a burn + a full reload (route.rs:943-945), so the auto-enable should be gated on !is_transcoding() or accepted as a one-time reload at start.  
  *Verified:* CONFIRMED. rust-modules/src/route.rs:657 `const PREF_AUDIO_LANG: &str = "eng";` is the only language preference in the crate, consumed once at route.rs:684 in `pick_dp_audio`'s rung 1. No subtitle-language preference exists; subs start Off unconditionally (route.rs:746 + player::reset_subtitle) and the only caller that ever sets a positive subtitle index is the user's own menu commit (commit_subtitle_selection). plex/account.rs implements exactly five plex.tv calls — create_pin (:73), poll_pin (:79), resources (:88), home_users (:96), switch_user (:104) — no /api/v2/user, no preferences. `forc
...
- **No subtitle search/download, and the detail page lists audio languages but not subtitle languages** — `polish` / `medium`  
  ...
  *Verified:* CONFIRMED both halves. No subtitle-search endpoint anywhere in rust-modules/src/plex (library.rs has metadata/children/all_leaves/select_streams/scrobble/unscrobble; account.rs has only the five plex.tv calls). Detail About block: rust-modules/src/ui/detail.rs:1446-1457 builds `orig_audio` from `d.audio.first()` and `audio_list` from `d.audio.iter().take(8)` as 'Lang (CODEC)'; d.subs is consulted only at :1459-1460 for the `cc`/`sdh` booleans that feed the Accessibility chips (:1462-1467), and the drawn Languages column (:1575-1583) has only 'Original Audio' + 'Audio'. metadata::Detail does ca

...
- **No subtitle timing (sync) offset** — `major` / `small`  
  ...
  *Landed (lands #243):* a **Timing** section under Color in the player's Subtitles menu (`ui::track_menu`) — Earlier / Later in 100 ms steps and Reset, the offset as the section's read-out, 5 s early to 30 s late — persisted as the public session preference `subtitle_offset_ms`. Every client-rendered lookup (embedded text, image sets, the sidecar) goes through `player::subtitle_clock_ns` (the playhead less the offset). Hidden on a transcode, where the server has already burned the captions. An embedded track's advance is bounded by the demuxer's read-ahead, which is why the early side stops at 5 s.  

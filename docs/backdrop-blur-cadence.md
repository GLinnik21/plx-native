...
Historical baseline for the now-retired dynamic Account policy: "The Account glass costs 60 fps →
45–50 fps" was measured with the timer profiler armed; with nothing armed, that glass itself cost
zero frames — 60.1 and 59.8 against a glass-absent control of 59.9 and 60.1. Account now freezes its
completed host in `gfx::FrameCache` and uses cached panel glass, so it has no recurring cadence.

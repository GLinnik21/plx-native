...
**What it writes.** Persistent files are mode 0600. The packaged `state/` directory is the one
exception: it is shipped empty as uid 0, gid 5000, mode 0775 so the jailed app can create its own
0600 files there.
...
- `state/auth.json` — the app-local fallback session file inside the packaged `state/` directory. The directory is shipped empty with uid 0, gid 5000 and mode 0775; the file itself is created owner-only at mode 0600. It is used only after the external candidates above are unavailable or unusable.
- `state/telemetry.json` — the app-local fallback for the reporting decision, with the same empty packaged directory and 0600 runtime-file rule. Its spool and crashmark companions use the same `state/` directory and are not package data.
...
| `state/auth.json` — app-local session fallback | `session::save`/`update` via `write_atomic` | 0600 inside packaged `state/` | same ownership, mode and quarantine rules as the external session candidates |
...
| `state/telemetry.json` — app-local consent fallback | `telemetry::record` via `write_atomic` | 0600 inside packaged `state/` | same ownership and reset rules as the external decision candidates |

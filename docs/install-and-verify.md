...
- The canonical persistent destinations are `state/session.json` and `state/consent.json`. The legacy names described below are migration inputs only; current writes use versioned Records and `.session.json.new-*`/`.consent.json.new-*` temporary files.
...
- `state/session.json` — the canonical versioned session record. Legacy `<id>-auth.json` and `state/auth.json` files are migration sources only. The directory is shipped empty with uid 0, gid 5000 and mode 0775; the record is created owner-only at mode 0600.
- `state/consent.json` — the canonical reporting-decision record. Legacy telemetry decision paths are migration sources only; its spool and crashmark companions use the same `state/` directory and are not package data.

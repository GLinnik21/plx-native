...
| ~~`original_feature_bonus`~~ | ~~25~~ | **SUPERSEDED, landed I7b.** The row's own demand was met by the plan's §6.2 three-way split — `dv_bonus` 13 / `generation_loss_bonus` 8 / `atmos_bonus` 4, rank weights over the same total, ordering load-bearing and magnitudes explicitly not. The constant no longer exists. |
